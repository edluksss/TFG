...
if __name__ == "__main__":
    ...
    model_name = "FCCN_simple_window_simpler_dice_tanh_512_th0.9"
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset_train)):
        ...
        model = ConvNet(input_dim = 1, hidden_dims = [4, 4, 4], output_dim = 1, transposeConv=False, separable_conv=False, activation_layer=activation_layer, kernel_size = 9, padding = 'same')
        
        # Definimos el modelo con los pesos inicializados aleatoriamente (sin preentrenar)
        model = smpAdapter(model = model, learning_rate=lr, threshold=0.9, current_fold=fold, loss_fn=loss_fn, scheduler=None)
